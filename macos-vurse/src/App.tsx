...
const protectionModules = [
  {
    ...
    copy: "DNS protection and suspicious connections",
    icon: Network,
    value: "14 blocked",
  },
  {
    title: "Link & QR Scanner",
    copy: "Inspect redirects before you open them",
    icon: QrCode,
    value: "Ready",
  },
  {
    title: "File Check",
    copy: "Signature and behavior inspection",
    icon: FileCheck2,
    value: "No threats",
  },
  {
    title: "Clipboard Guard",
    copy: "Payment destinations stay unchanged",
    icon: Copy,
    value: "Watching",
  },
  {
    title: "Remote Access Guard",
    copy: "Screen-control requests require verification",
    icon: Airplay,
    value: "Protected",
  },
  {
    title: "Login Guard",
    copy: "Review new devices and recovery changes",
    icon: KeyRound,
    value: "1 reviewed",
  },
];
...
const initialFamilyContacts: FamilyContact[] = [
  {
    ...
    baseline: "Never requests gift cards",
    safePhrase: true,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    baseline: "No money requests",
    safePhrase: false,
  },
];

const NO_CONTACT_MATCH =
  "Unknown — no name matching your trusted circle was found";
...
function matchClaimedSender(
  ...
  for (const { name } of contacts) {
    ...
    const fullRe = new RegExp(
      `\\b${escapeRegex(name).replace(/\s+/g, "\\s+")}\\b`,
      "i",
    );
    if (fullRe.test(text) || firstRe.test(text)) matched.push(name);
  }
  if (matched.length === 0)
    return { display: NO_CONTACT_MATCH, firstName: "the caller", count: 0 };
  if (matched.length === 1)
    return {
      display: matched[0],
      firstName: matched[0].split(" ")[0],
      count: 1,
    };
  return {
    display: matched.join(", "),
    firstName: "the caller",
...
function App() {
  ...
  const [verification, setVerification] =
    useState<VerificationState>("RECEIVED");
  const [interruption, setInterruption] = useState(false);
  const [receiptReady, setReceiptReady] = useState(false);
  const [trustedReply, setTrustedReply] = useState(false);
  const [contacts, setContacts] = useState<FamilyContact[]>(
    initialFamilyContacts,
  );
  const [claimedMatch, setClaimedMatch] = useState<ClaimMatch>(() =>
    matchClaimedSender("", initialFamilyContacts),
  );
  ...
  const [notifications, setNotifications] = useState<Notice[]>([
    {
      id: 1,
      title: "Protected",
      body: "Identity monitoring and Network Shield are active.",
      level: "safe",
      page: "overview",
    },
  ]);
  ...
  const pushNotice = (
    title: string,
    body: string,
    level: Notice["level"] = "attention",
    targetPage: Page = "overview",
  ) => {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      pushNotice(
        "Verus is quietly watching",
        "Open Secure Mail in Safari to try the phishing interruption demo.",
        "safe",
        "verify",
      );
    }, 4200);
  ...
  const isolateNetwork = () => {
    ...
    pushNotice(
      "Network isolated",
      "This simulated Mac is no longer connected to the internet.",
      "attention",
      "protection",
    );
  };
  ...
  const restoreNetwork = () => {
    ...
    pushNotice(
      ...
      "Network Shield is active and the connection is trusted.",
      "safe",
      "overview",
    );
  };
  ...
  const launchScenario = (
    ...
    const messageForMatch =
      message ??
      scenarioMessages[name] ??
      scenarioMessages[name.replace(/ scam$/i, "")] ??
      "";
    ...
    if (name.includes("Public")) {
      setPublicWifi(true);
      openVerus("protection");
      pushNotice(
        "Public Wi-Fi Mode active",
        "Untrusted-network protections were enabled automatically.",
        "attention",
        "protection",
      );
      return;
    }
    if (name.includes("Data broker")) {
      openVerus("privacy");
      pushNotice(
        "Exposure found",
        "An old address and phone number were found on a people-search site.",
        "attention",
        "privacy",
      );
      return;
    }
    if (name.includes("Disconnect")) {
      isolateNetwork();
      return;
    }
    if (name.includes("Recovery takeover")) {
      openVerus("recovery");
      startLockdown();
      return;
    }
    if (name.includes("Legitimate")) {
      setVerification("PAUSED");
      setTrustedReply(false);
      setReceiptReady(false);
      setInterruption(true);
      pushNotice(
        "Action paused",
        "A family request is waiting for independent confirmation.",
        "attention",
        "verify",
      );
      return;
    ...
    pushNotice(
      "Action paused",
      `${name} is waiting for independent confirmation.`,
      "danger",
      "verify",
    );
  };
  ...
  const openScamEmail = () => {
    ...
    window.setTimeout(() => {
      ...
      pushNotice(
        ...
        "This message uses a lookalike sign-in page and account pressure.",
        "danger",
        "verify",
      );
    }, 2000);
  ...
  const startVerification = () => {
    ...
    window.setTimeout(() => setTrustedReply(true), 900);
  };

  const resolveVerification = (approved: boolean) => {
    setVerification(approved ? "VERIFIED" : "DENIED");
    setTrustedReply(false);
    setReceiptReady(true);
    pushNotice(
      approved ? "Verification complete" : "Request denied",
      approved
        ? `${claimedMatch.firstName} confirmed that this request is legitimate.`
        : `${claimedMatch.firstName} reported that they did not send this request.`,
      approved ? "safe" : "danger",
      "activity",
    );
  };
  ...
  const closeInterruption = () => {
    if (!["DENIED", "VERIFIED", "EXPIRED"].includes(verification)) {
      pushNotice(
        "Request remains paused",
        "The risky action cannot continue before independent confirmation.",
        "attention",
        "verify",
      );
      return;
    ...
    openVerus(receiptReady ? "activity" : "verify");
  };

  const checkIdentity = async () => {
    setCheckingIdentity(true);
    setRecoveryPlan(false);
    try {
      const r = await fetch(`/api/breaches?email=${encodeURIComponent(email)}`);
      if (!r.ok) throw new Error();
      const d = await r.json();
      setBreaches(d.breaches ?? []);
      setBreachSource(d.source === "hibp" ? "hibp" : "demo");
    } catch {
      setBreaches(demoBreaches);
      setBreachSource("demo");
    } finally {
      setCheckingIdentity(false);
      pushNotice(
        "Credential exposure found",
        "Older breach records were found. Credential reuse is blocked until reviewed.",
        "attention",
        "identity",
      );
    }
  ...
  const startLockdown = () => {
    ...
    pushNotice(
      ...
      "Network, outgoing payments, sessions, and the recovery vault are paused.",
      "danger",
      "recovery",
    );
  };
  ...
  return (
    <main className={bodyClass}>
        ...
        {menuOpen && (
          <VerusPopover
            ...
            onPrivacy={() => {
              setPrivacyMode((v) => !v);
              pushNotice(
                privacyMode ? "Privacy Mode off" : "Privacy Mode active",
                privacyMode
                  ? "Standard connection settings restored."
                  : "Trackers and nonessential background connections are paused.",
                "safe",
                "privacy",
              );
            }}
      ...
      <section className="desktop-icons" aria-label="Desktop shortcuts">
        <DesktopShortcut
          label="Safety Receipts"
          icon={<FileCheck2 />}
          onClick={() => openVerus("activity")}
        />
        <DesktopShortcut
          label="Recovery Vault"
          icon={<Vault />}
          onClick={() => openVerus("recovery")}
        />
      </section>
          ...
          >
            {activeApp === "verus" && (
              <VerusApp
                ...
                claimedSender={claimedMatch.display}
                email={email}
                setEmail={setEmail}
                breaches={breaches}
                ...
                onRecoveryPlan={() => {
                  setRecoveryPlan(true);
                  pushNotice(
                    "Recovery plan created",
                    "Start with the primary email account, then update dependent accounts.",
                    "safe",
                    "recovery",
                  );
                }}
                onPrivacyMode={() => {
                  ...
                  pushNotice(
                    privacyMode
                      ? "Disappear Mode off"
                      : "Disappear Mode active",
                    privacyMode
                      ? "Standard privacy controls restored."
                      : "Nonessential exposure has been reduced.",
                    "safe",
                    "privacy",
                  );
                }}
                onDisconnect={
                  networkConnected ? isolateNetwork : restoreNetwork
                }
                onLockdown={lockdown ? restoreNetwork : startLockdown}
                onHighContrast={setHighContrast}
                onLargeText={setLargeText}
                onToast={(msg) =>
                  pushNotice(msg, "Demo action completed.", "safe", page)
                }
              />
            )}
            {activeApp === "safari" && (
              <SafariApp
                ...
                onOpenEmail={openScamEmail}
              />
            )}
            {activeApp === "messages" && (
              <MessagesApp
                onVerify={() => launchScenario("Gift-card request")}
              />
            )}
          </AppWindow>
        )}
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            ...
            <div>
              <strong>{toast}</strong>
              <span>Click the Verus icon for details.</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {interruption && (
          <InterruptionOverlay
            state={verification}
            trustedReply={trustedReply}
            receiptReady={receiptReady}
            claimedSender={claimedMatch.display}
            claimedFirstName={claimedMatch.firstName}
            onStart={startVerification}
            onResolve={resolveVerification}
            onClose={closeInterruption}
            onEnd={() => {
              setVerification("DENIED");
              setReceiptReady(true);
              pushNotice(
                "Interaction ended safely",
                "No money or credentials were sent.",
                "safe",
                "activity",
              );
            }}
...
function MacMenuBar({
  ...
  const menus = activeApp
    ? (appMenus[activeApp] ?? appMenus.verus)
  ...
  return (
    <header className="menu-bar">
      <div className="menu-left">
        ...
        {menus.map((item, i) => (
          <span key={item} className={i === 0 ? "menu-app-name" : "menu-item"}>
            {item}
          </span>
        ))}
      </div>
      <div className="menu-right">
        ...
        >
          <Bell size={14} />
          {notificationCount > 0 && (
            <span className="notice-count">{notificationCount}</span>
          )}
        </button>
        <Search size={14} className="menu-icon" />
        {connected ? (
          <Wifi size={14} className="menu-icon" />
        ) : (
          <WifiOff size={14} className="menu-icon danger-icon" />
        )}
...
function MacDock({
  ...
  const [pointer, setPointer] = useState<{ x: number; width: number } | null>(
    null,
  );
  ...
  const items: DockItem[] = [
    { id: "finder", label: "Finder", icon: <PanelLeft />, cls: "finder" },
    {
      id: "launchpad",
      label: "Launchpad",
      icon: <img src="/macos/launchpad.png" alt="" />,
      cls: "launchpad",
    },
    {
      id: "safari",
      label: "Safari",
      icon: <img src="/macos/safari.png" alt="" />,
      cls: "safari",
      action: () => onOpen("safari"),
    },
    {
      id: "messages",
      label: "Messages",
      icon: <img src="/macos/messages.png" alt="" />,
      cls: "messages",
      action: () => onOpen("messages"),
    },
    {
      id: "mail",
      label: "Mail",
      icon: <img src="/macos/mail.png" alt="" />,
      cls: "mail",
    },
    {
      id: "maps",
      label: "Maps",
      icon: <img src="/macos/maps.png" alt="" />,
      cls: "maps",
    },
    {
      id: "photos",
      label: "Photos",
      icon: <img src="/macos/photos.png" alt="" />,
      cls: "photos",
    },
    {
      id: "music",
      label: "Music",
      icon: <img src="/macos/music.png" alt="" />,
      cls: "music",
    },
    {
      id: "tv",
      label: "TV",
      icon: <img src="/macos/tv.png" alt="" />,
      cls: "tv",
    },
    {
      id: "news",
      label: "News",
      icon: <img src="/macos/news.png" alt="" />,
      cls: "news",
    },
    {
      id: "calendar",
      label: "Calendar",
      icon: <img src="/macos/calendar.png" alt="" />,
      cls: "calendar",
    },
    { sep: true, id: "s1" },
    {
      id: "verus",
      label: "Verus",
      icon: <img src="/verus-logo.png" alt="" />,
      cls: "verus",
      action: () => onOpen("verus"),
    },
    { sep: true, id: "s2" },
    {
      ...
      icon: <img src="/macos/systempreferences.png" alt="" />,
      cls: "settings",
    },
  ...
  return (
    ...
    >
      {items.map((item) => {
        ...
        const idx = nonSepItems.findIndex(
    ...
    >
      ...
      <div className="device-summary">
        ...
        <StatusLine
          ...
          text={connected ? "Network trusted" : "Network isolated"}
        />
      ...
      <div className="popover-section">
        ...
        <div className="quick-grid">
          <button onClick={onScenario}>
            <MessageCircle /> Check a message
          </button>
          <button onClick={() => onOpen("protection")}>
            <Link2 /> Scan a link
          </button>
          <button onClick={() => onOpen("identity")}>
            <KeyRound /> Check exposure
          </button>
          <button onClick={onPrivacy}>
            <EyeOff /> {privacyMode ? "End Privacy Mode" : "Start Privacy Mode"}
          </button>
        </div>
      </div>
      <div className="popover-section">
        ...
        <ul className="recent-list">
          <li>
            <span className="event-dot amber" /> Unknown login blocked
            <time>8m</time>
          </li>
          <li>
            <span className="event-dot green" /> Trusted contact confirmed
            <time>1h</time>
          </li>
          <li>
            <span className="event-dot purple" /> 3 tracking requests blocked
            <time>2h</time>
          </li>
        </ul>
      </div>
      <button className="disconnect-button" onClick={onDisconnect}>
        ...
        <span>
          <strong>
            {connected ? "Disconnect This Mac" : "Restore Connection"}
          </strong>
          <small>
            {connected
              ? "Simulate a network kill switch"
              : "Return to trusted network"}
          </small>
        </span>
      </button>
      <button
      ...
      >
        Open Verus <ArrowRight />
      </button>
      <p className="privacy-note">
        <Lock size={12} /> Everything is processed privately whenever possible.
      </p>
    </motion.aside>
  );
}
    ...
    >
      ...
      {notices.length === 0 ? (
        ...
        notices.map((n) => (
          <button
            key={n.id}
            className={`notification-card ${n.level}`}
            onClick={() => onOpen(n)}
          >
            <img src="/verus-logo.png" alt="" />
            <span>
              <small>VERUS · now</small>
...
function DesktopShortcut({
  ...
  return (
    <button onDoubleClick={onClick} onClick={onClick}>
      <span>{icon}</span>
      {label}
    </button>
  );
...
type VerusAppProps = {
  ...
  onScenario: (name?: string, outcome?: string, message?: string) => void;
      ...
      <aside className="sidebar">
        ...
        <nav>
          {navItems.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              className={props.page === id ? "active" : ""}
              onClick={() => props.onPage(id)}
            >
  ...
  const cards = [
    {
      ...
      value: "Protected",
      detail: "14 risky connections blocked",
      meta: "Last scan 8 minutes ago",
      icon: Laptop,
    },
    {
      label: "Identity Exposure",
      value: "Monitoring",
      detail: "2 old breaches found",
      meta: "No active abuse detected",
      icon: Fingerprint,
    },
    {
      label: "Verification Requests",
      value: props.verification === "PAUSED" ? "Action needed" : "Ready",
      detail: "1 request verified today",
      meta: "0 unresolved",
      icon: BadgeCheck,
    },
    {
      label: "Privacy",
      value: props.privacyMode ? "Disappear Mode" : "Protected",
      detail: "23 trackers blocked today",
      meta: "4 removals active",
      icon: EyeOff,
    },
  ];
  return (
    <>
      <section className="hero-card">
        <div className="hero-copy">
          ...
          <p>
            Verus is independently checking risky requests before anything
            irreversible happens.
          </p>
          <div className="hero-actions">
            <button
              className="primary-button"
              onClick={() => props.onPage("verify")}
            >
              Verify something <ArrowRight />
            </button>
            <button
              className="secondary-button"
              onClick={() => props.onToast("Safety check complete")}
            >
              <ScanLine /> Run safety check
            </button>
          </div>
        </div>
      ...
      <section className="chart-card">
        <div className="section-title">
          <div>
            ...
            <p>Quiet events, important interruptions, and what Verus did.</p>
          </div>
        ...
        <div className="chart-wrap">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart
              data={activityData}
              margin={{ top: 12, right: 18, left: -28, bottom: 0 }}
            >
              <CartesianGrid stroke="rgba(255,255,255,.06)" vertical={false} />
              <XAxis
                dataKey="time"
                tick={{ fill: "#858aa0", fontSize: 12 }}
                axisLine={false}
                tickLine={false}
              />
...
function ActivityTooltip({
  ...
  return (
    <div className="chart-tooltip">
      ...
      <p>Risk: {item.risk}</p>
      <span>Verus checked the event. No user action required.</span>
    </div>
  );
}
...
const scenarioMessages: Record<string, string> = {
  "Grandchild emergency":
    "This is Daniel. I lost my phone. Please send $1,200 for bail and do not tell Mom.",
  "Bank fraud call":
    "This is the fraud department at your bank. We detected suspicious activity. To stop the transfer, read us the one-time verification code we just texted you and confirm your account number right now.",
  "Executive impersonation":
    "Hi, it's the CEO. I need you to process an urgent wire transfer of $48,000 to a new vendor before the end of the day. Keep this confidential until the deal is announced.",
  "Legitimate emergency":
    "Hi Mom, it's Daniel calling from my own phone. Running late after practice so I'll miss dinner — no need to do anything, see you tonight.",
};
...
function VerifyPage(props: VerusAppProps) {
  ...
  const [tab, setTab] = useState<"message" | "link" | "phone" | "screenshot">(
    "message",
  );
  ...
  return (
    <>
      <PageHeading
        ...
        title="Do not guess. Verify."
        copy="Check the person, request, destination, or account before you act."
      >
        <span className={`state-pill ${props.verification.toLowerCase()}`}>
          {props.verification}
        </span>
      </PageHeading>
      <div className="verify-layout">
        <section className="panel verify-input-panel">
          <div className="input-tabs">
            ...
            <button
              className={tab === "screenshot" ? "active" : ""}
          ...
          {tab === "phone" && (
            <p className="explain-note">
              We don&rsquo;t try to detect cloned voices &mdash; even trained
              listeners get this wrong most of the time. We verify through
              something a clone can&rsquo;t know instead.
            </p>
          )}
          ...
          <div className="privacy-strip">
            <Lock /> Processed temporarily · Message content is not saved
          </div>
          <button
            className="primary-button full"
            onClick={() => {
              if (analysis.length === 0) {
                props.onToast(
                  "No known scam patterns matched. Verify directly if anything feels off.",
                );
                return;
              }
        ...
        <section className="panel signal-panel">
          ...
          {analysis.length === 0 ? (
            <p className="explain-note">
              No known scam patterns matched this message. That isn&rsquo;t a
              guarantee it is genuine &mdash; if anything feels off, verify
              directly with the person using a number you already trust.
            </p>
          ) : (
            <>
              <p className="analysis-summary">
                {analysis.length} risk signal{analysis.length === 1 ? "" : "s"}{" "}
                found in this message.
              </p>
              {facts.length > 0 && (
                <div className="evidence-group">
                  <div className="evidence-heading">
                    <Fingerprint /> Deterministic facts
                  </div>
                  {facts.map((f) => (
                    <SignalTag
                      key={f.category}
                      strong
                      text={`${f.category}: “${f.evidence}”`}
                    />
                  ))}
                </div>
              )}
              {signals.length > 0 && (
                <div className="evidence-group">
                  <div className="evidence-heading">
                    <Sparkles /> Model-assisted signals
                  </div>
              ...
              <p className="explain-note">
                Language signals explain why Verus paused the action. They are
                not presented as confirmed facts.
              </p>
            </>
          )}
        </section>
      ...
      <section className="panel scenario-row">
        <div>
          <small>PRELOADED DEMOS</small>
          <h2>Try a complete verification flow</h2>
        </div>
        <div className="scenario-buttons">
          {[
            ...
            <button
              key={name}
              onClick={() => {
                const msg = scenarioMessages[name];
                if (msg) setInput(msg);
                props.onScenario(name, undefined, msg);
              }}
            >
              <ShieldAlert /> {name}
            </button>
...
function SignalTag({
  ...
  return (
    <span className={`signal-tag ${strong ? "fact" : ""}`} title={title}>
      {strong ? <CheckCircle2 /> : <AlertTriangle />} {text}
    </span>
  );
}
  ...
  return (
    <>
      <PageHeading
        ...
        copy="Calm, layered protection for links, files, sessions, and connections."
      >
      ...
      {!props.connected && (
        <div className="isolation-banner">
          <WifiOff />
          <div>
            <strong>Mac isolated from the internet</strong>
            <p>
              Simulated network access is paused. Local Verus controls remain
              available.
            </p>
          </div>
      ...
      {props.publicWifi && (
        <div className="attention-banner">
          <ShieldAlert />
          <div>
            <strong>Public Wi-Fi Mode is active</strong>
            <p>
              Browser isolation, encrypted DNS, and tracker blocking are
              enabled.
            </p>
          </div>
        </div>
      ...
      <div className="module-grid">
        {protectionModules.map(({ title, copy, value, icon: Icon }) => (
          <article className="module-card" key={title}>
            ...
            <strong>{value}</strong>
            <button onClick={() => props.onToast(`${title} checked`)}>
              Review <ChevronRight />
            </button>
          </article>
        ))}
      ...
      <section className="panel link-scanner">
        <div>
          <small>LINK & QR SCANNER</small>
          <h2>Check a destination before opening it</h2>
        </div>
        <div className="scanner-input">
          ...
          <button
            onClick={() =>
              setScanResult(
                "Blocked: newly created lookalike domain redirects to an unrelated sign-in page.",
              )
            }
          >
            Scan
        ...
        {scanResult && (
          <div className="scan-result">
            <TriangleAlert />
            <div>
              <strong>Destination blocked</strong>
              <p>{scanResult}</p>
            </div>
          </div>
        )}
  ...
  return (
    <>
      <PageHeading
        ...
        title="Know where you were exposed."
        copy="Check older breach records, then secure the accounts that can reset everything else."
      >
        <span className="safe-pill">
          <Fingerprint /> Monitoring active
        </span>
      </PageHeading>
      <section className="identity-hero panel">
        <div className="identity-check">
          ...
          <p>
            Live HIBP checks run only when a server-side API key is configured.
            Demo mode never presents generated results as live findings.
          </p>
          ...
          <div className="identity-input">
            ...
            />
            <button
              onClick={props.onCheckIdentity}
              disabled={props.checkingIdentity}
            >
              {props.checkingIdentity ? (
          ...
          <span className="not-saved">
            <Lock /> Email is used for this check and is not stored by this
            prototype.
          </span>
        </div>
        <div className="account-map">
          ...
          <div className="account-map-visual">
            <span className="account-node core">
              <Mail /> Gmail
            </span>
            <span className="account-node n1">
              <Banknote /> Bank
            </span>
            <span className="account-node n2">
              <Cloud /> Cloud
            </span>
            <span className="account-node n3">
              <WalletCards /> Shopping
            </span>
            <span className="account-node n4">
      ...
      {props.breaches && (
        <section className="breach-results">
          <div className="section-title">
            <div>
              <small>
                {props.breachSource === "hibp"
                  ? "LIVE HIBP RESULT"
                  : "LABELED DEMO DATA"}
              </small>
              <h2>
                {props.breaches.length
                  ? `${props.breaches.length} older exposures found`
                  : "No exposure found in the returned data"}
              </h2>
              <p>
                Credential reuse is blocked in this demo until the recovery
                steps are reviewed.
              </p>
            </div>
            {props.breaches.length > 0 && (
              <button className="primary-button" onClick={props.onRecoveryPlan}>
                Create recovery plan <ArrowRight />
              </button>
            )}
          </div>
          {props.breaches.map((breach) => (
            <article className="breach-card" key={breach.Name}>
              ...
              <div className="breach-actions">
                <span>
                  <Check /> Password reset planned
                </span>
                <span>
                  <Shield /> 2FA review needed
                </span>
              </div>
            </article>
          ))}
          {props.recoveryPlan && (
            <div className="recovery-plan">
              <CheckCircle2 />
              <div>
                <strong>Recovery plan ready</strong>
                <p>
                  1. Secure Gmail · 2. Revoke old sessions · 3. Change unique
                  passwords · 4. Review recovery methods
                </p>
              </div>
            </div>
          )}
  ...
  return (
    <>
      <PageHeading
        ...
        copy="Reduce unnecessary exposure without disappearing from the services you need."
      >
        <button
          className={`mode-switch ${props.privacyMode ? "on" : ""}`}
          onClick={props.onPrivacyMode}
          aria-pressed={props.privacyMode}
        >
          <span />
          {props.privacyMode ? "Active" : "Off"}
        </button>
      </PageHeading>
      <div className="privacy-grid">
        <section className="panel privacy-controls">
          <div className="privacy-illustration">
            <EyeOff />
            <span>
              {props.privacyMode ? "Exposure reduced" : "Ready when you are"}
            </span>
          </div>
          {[
            "Block known trackers",
            "Hide simulated IP details",
            "Pause advertising identifiers",
            "Prevent clipboard monitoring",
            "Limit camera and microphone access",
            "Isolate the simulated browser session",
          ].map((item) => (
            <div className="control-row" key={item}>
              <CheckCircle2 />
              <span>{item}</span>
              <strong>{props.privacyMode ? "On" : "Ready"}</strong>
            </div>
          ))}
        </section>
      ...
      <section className="panel removal-panel">
        <div>
          <small>REMOVE ME FROM THE INTERNET</small>
          <h2>Reduce data-broker and people-search exposure</h2>
          <p>
            This is exposure reduction, not a promise to erase the entire
            internet.
          </p>
        </div>
        <div className="removal-stats">
          <span>
            <strong>12</strong> sources found
          </span>
          <span>
            <strong>{removal ? 5 : 4}</strong> requests submitted
          </span>
          <span>
            <strong>3</strong> removals confirmed
          </span>
        </div>
        <button
          className="primary-button"
          onClick={() => {
            setRemoval(true);
            props.onToast("Removal request started");
          }}
        >
          {removal ? <Check /> : <Trash2 />}
          {removal ? "Request submitted" : "Start next removal"}
        </button>
      </section>
    </>
  );
...
function RecoveryPage(props: VerusAppProps) {
  const playbooks = [
    ...
    "My device may be compromised",
    "I think I was fooled by a cloned voice or fake caller",
  ...
  return (
    <>
      <PageHeading
        ...
        title="Get safe, one step at a time."
        copy="Contain the damage first. Then secure the accounts that matter most."
      >
        <button
          className={`lockdown-button ${props.lockdown ? "active" : ""}`}
          onClick={props.onLockdown}
        >
          <Power />{" "}
          {props.lockdown
      ...
      {props.lockdown && (
        <section className="lockdown-status">
          <ShieldAlert />
          <div>
            <small>EMERGENCY LOCKDOWN ACTIVE</small>
            <h2>Risky activity is contained.</h2>
            <p>
              Network disconnected · payments paused · sessions revoked · vault
              locked · trusted contacts notified
            </p>
          </div>
      ...
      <div className="playbook-grid">
        {playbooks.map((title, i) => (
          <button
            className="playbook"
            key={title}
            onClick={() => props.onToast(`${title} playbook opened`)}
          >
            <span>{String(i + 1).padStart(2, "0")}</span>
            <div>
              <strong>{title}</strong>
              <p>Contain · Secure · Revoke · Recover</p>
            </div>
            <ChevronRight />
          </button>
        ))}
      ...
      <section className="panel vault-panel">
        <div className="vault-heading">
          ...
          <div>
            <small>RECOVERY VAULT</small>
            <h2>Sensitive details stay hidden</h2>
            <p>Fictional demo values · automatic re-hiding enabled</p>
          </div>
        </div>
        ...
        <p className="explain-note">
          These details stay on this device and are never sent to our servers or
          to the AI model. (Demo build: the values above are fictional
          placeholders, not yet backed by real on-device encryption.)
        </p>
      </section>
    </>
  );
...
function VaultRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="vault-row">
      ...
      <button onClick={() => setRevealed((c) => !c)}>
...
function FamilyPage(props: VerusAppProps) {
  ...
  const submitContact = (event: React.FormEvent) => {
    ...
    if (!form.phone.trim() && !form.email.trim()) {
      setError("Add at least one contact method — a phone number or email.");
      return;
    }
    props.onAddContact({
      name: form.name.trim(),
  ...
  return (
    <>
      <PageHeading
        eyebrow="TRUSTED CONTACT NETWORK"
        title="Family Circle"
        copy="Independent confirmation comes from people and contact methods you enrolled in advance."
      >
        <button
          className="primary-button"
          onClick={() => {
      ...
      {formOpen && (
        <form className="contact-form panel" onSubmit={submitContact}>
          ...
          <input
            ...
            value={form.baseline}
            onChange={(e) =>
              setForm((f) => ({ ...f, baseline: e.target.value }))
            }
            placeholder="Baseline note (optional, e.g. Never requests gift cards)"
            aria-label="Baseline note"
          />
          <label className="contact-form-check span-2">
            <input
              type="checkbox"
              checked={form.safePhrase}
              onChange={(e) =>
                setForm((f) => ({ ...f, safePhrase: e.target.checked }))
              }
            />{" "}
            Safe phrase enrolled for this contact
          </label>
        ...
        {props.contacts.map((contact) => (
          <article className="contact-card" key={contact.id}>
            ...
            <span className="verified-label">
              <BadgeCheck /> Independently verified
            </span>
            {contact.phone && (
            ...
            <button
              onClick={() =>
                props.onToast(`${contact.name} verification request sent`)
              }
            >
              Send test request <ArrowRight />
            </button>
            {contact.safePhrase && (
              <span className="safe-phrase">Safe phrase enrolled</span>
            )}
            {contact.safePhrase && (
              <small className="safe-phrase-note">
                Not on social media. Not something a clone could learn from a
                voice sample.
              </small>
            )}
              ...
              <div className="contact-remove-confirm">
                ...
                <div>
                  <button
                    className="danger"
                    onClick={() => {
                      props.onRemoveContact(contact.id);
                      props.onToast(
                        `${contact.name} removed from Family Circle`,
                      );
                      setConfirmId(null);
                    }}
                  >
      ...
      <section className="panel rehearsal-panel">
        <div>
          <small>SAFETY REHEARSAL</small>
          <h2>Practice before a real urgent moment.</h2>
          <p>
            A message claims Maya lost her phone and urgently needs $600 in gift
            cards.
          </p>
        </div>
        <div className="rehearsal-steps">
          {[
            "Notice the changed number",
            "Pause the request",
            "Contact Maya using the saved number",
            "Ask the safe-phrase question",
            "Deny the false request",
          ].map((step, i) => (
            <button
              key={step}
              className={i <= rehearsal ? "done" : ""}
              onClick={() => setRehearsal(Math.min(i + 1, 4))}
            >
              {i < rehearsal ? <Check /> : <span>{i + 1}</span>}
              {step}
            </button>
          ))}
        </div>
        <button
          ...
          onClick={() => {
            setRehearsal(4);
            props.onToast("Safety rehearsal complete");
          }}
        >
          Complete rehearsal <BookOpenCheck />
        </button>
      </section>
    </>
  );
  ...
  return (
    <>
      <PageHeading
        ...
        title="What Verus did, in plain language."
        copy="Behavioral data is device-local, minimally retained, and deletable."
      >
        <button className="secondary-button" onClick={() => setDeleted(true)}>
          <Trash2 /> Delete behavior data
        </button>
      </PageHeading>
      {deleted ? (
        <section className="empty-state panel">
          <Trash2 />
          <h2>Behavior data deleted</h2>
          <p>Safety Receipts remain until you delete them separately.</p>
          <button onClick={() => setDeleted(false)}>Restore demo data</button>
        </section>
      ) : (
        <div className="activity-layout">
          <section className="panel behavior-panel">
            <div className="behavior-tabs">
              <button className="active">Signals</button>
            ...
            <div className="baseline-grid">
              <div>
                <small>EXPLICIT BASELINE</small>
                <h3>What your family told Verus</h3>
                <p>Gift cards are never requested.</p>
                <p>Daniel uses one saved mobile number.</p>
                <p>Bank staff never ask for one-time codes.</p>
              </div>
              <div>
                <small>CURRENT RULES</small>
                <h3>When Verus will pause</h3>
                <p>Payment destination changes.</p>
                <p>A known person uses a new contact method.</p>
                <p>Remote-access software requests control.</p>
              </div>
            </div>
          </section>
          <section className="panel timeline-panel">
            ...
            ].map(([time, event, note, state]) => (
              <div className="timeline-row" key={event}>
                <span className={`event-dot ${state}`} />
                <time>{time}</time>
                <div>
                  <strong>{event}</strong>
                  <p>{note}</p>
      ...
      {props.receiptReady && (
        <section className="receipt panel">
          <div className="receipt-head">
            ...
            <div>
              <small>SAFETY RECEIPT · VR-260621-1048</small>
              <h2>Grandchild emergency request</h2>
              <p>
                The original message was analyzed temporarily and was not
                stored.
              </p>
            </div>
            <span className={`state-pill ${props.verification.toLowerCase()}`}>
          ...
          <div className="receipt-grid">
            <span>
              <small>CLAIMED SENDER</small>
              {props.claimedSender}
            </span>
            <span>
              <small>CONTACT METHOD</small>Unknown mobile number
            </span>
            <span>
              <small>SIGNALS</small>Urgency · secrecy · payment
            </span>
            <span>
              <small>DETERMINISTIC MISMATCH</small>Number not saved
            </span>
            <span>
              <small>VERIFICATION METHOD</small>Saved contact channel
            </span>
            <span>
              <small>ACTION</small>Payment blocked
            </span>
          </div>
          <div className="receipt-actions">
            <button onClick={() => props.onToast("PDF export prepared")}>
              Export PDF
            </button>
            <button onClick={() => props.onToast("Receipt shared with family")}>
              Share with family
            </button>
            <button onClick={() => props.onToast("Fraud report prepared")}>
              Report as fraud
            </button>
          </div>
        </section>
...
const initialEnterpriseMembers: EnterpriseMember[] = [
  {
    id: 1,
    name: "Sparsh Roy",
    email: "sparsh@northstar.example",
    role: "Owner",
    team: "Security",
    status: "Active",
    initials: "SR",
...
function EnterprisePage(props: VerusAppProps) {
  ...
  const inviteMember = (event: React.FormEvent) => {
    ...
    const name =
      inviteName.trim() || inviteEmail.split("@")[0].replace(/[._-]/g, " ");
    const initials =
      name
        .split(" ")
        .filter(Boolean)
  ...
  const policyCards: Array<{
  ...
  }> = [
    {
      ...
      copy: "Require the organization identity provider for access.",
      icon: KeyRound,
    },
    {
      id: "mfa",
      title: "Phishing-resistant MFA",
      copy: "Require a security key or platform passkey for admins.",
      icon: Fingerprint,
    },
    {
      id: "paymentVerification",
      title: "High-risk payment verification",
      copy: "Pause changed destinations and require a second approver.",
      icon: Banknote,
    },
    {
      id: "unmanagedDevices",
      title: "Block unmanaged devices",
      copy: "Limit organization data to enrolled and compliant devices.",
      icon: MonitorCheck,
    },
  ];
  ...
  return (
    <>
      <PageHeading
        eyebrow="VERUS ENTERPRISE"
        title="Organization security"
        copy="Manage people, policies, devices, and high-risk verification across your organization."
      >
        <button
          className="primary-button"
      ...
      <section className="enterprise-org-card">
        ...
        <span className="verified-org">
          <BadgeCheck /> Domain verified
        </span>
        <button
          className="secondary-button"
          onClick={() => props.onToast("Organization settings opened")}
        >
      ...
      <div className="enterprise-metrics">
        <article>
          ...
          <div>
            <small>PROTECTED PEOPLE</small>
            <strong>{activeMembers}</strong>
            <p>{members.length - activeMembers} awaiting action</p>
          </div>
        </article>
        <article>
          <span>
          ...
          <div>
            <small>ENROLLED DEVICES</small>
            <strong>7</strong>
            <p>1 needs an update</p>
          </div>
        </article>
        <article>
          <span>
        ...
        <article>
          ...
          <div>
            ...
            <strong>{policyCoverage}%</strong>
            <p>
              {Object.values(policies).filter(Boolean).length} of 4 enforced
            </p>
          </div>
        </article>
      </div>
      ...
      <div className="enterprise-workspace">
        <section className="panel enterprise-main">
            ...
            <>
              <div className="enterprise-section-head">
                <div>
                  ...
                  <p>
                    Assign roles, review enrollment, and suspend access
                    immediately.
                  </p>
                </div>
              ...
              {inviteOpen && (
                <form
                  className="enterprise-invite-form"
          ...
          {view === "policies" && (
            <>
              <div className="enterprise-section-head">
                <div>
                  <small>ORGANIZATION CONTROLS</small>
                  <h2>Security policies</h2>
                  <p>Changes apply to all assigned people and devices.</p>
                </div>
              ...
              <div className="enterprise-policy-grid">
                {policyCards.map(({ id, title, copy, icon: Icon }) => (
                  <article key={id}>
                    ...
                    <button
                      className={`toggle ${policies[id] ? "on" : ""}`}
                      onClick={() => togglePolicy(id)}
                      aria-pressed={policies[id]}
                    >
                      <span />
                    </button>
                  </article>
            ...
            <>
              <div className="enterprise-section-head">
                <div>
                  ...
                  <h2>Audit log</h2>
                  <p>Review access, policy, and verification changes.</p>
                </div>
                <button
                  onClick={() => props.onToast("Audit CSV export prepared")}
                >
                  <Download /> Export CSV
                </button>
              </div>
              <div className="enterprise-audit-list">
                {[
                  [
                    "Policy updated",
                    "Phishing-resistant MFA enabled for Administrators",
                    "Sparsh Roy",
                    "2 min ago",
                  ],
                  [
                    "Risky action contained",
                    "Changed vendor bank details paused pending approval",
                    "Verus",
                    "18 min ago",
                  ],
                  [
                    "Member invited",
                    "Elena Garcia invited to Operations",
                    "Maya Chen",
                    "1 hr ago",
                  ],
                  [
                    "Device enrolled",
                    "MacBook Pro · compliant",
                    "Noah Williams",
                    "Yesterday",
                  ],
        ...
        <aside className="enterprise-side">
          <section className="panel enterprise-posture">
            ...
            <p>
              Enable unmanaged-device blocking to close the remaining control
              gap.
            </p>
            <button
              onClick={() => {
                ...
                setPolicies((current) => ({
                  ...current,
                  unmanagedDevices: true,
                }));
                props.onToast("Unmanaged-device blocking enabled");
              }}
            >
              Fix recommendation <ArrowRight />
            </button>
          </section>
          <section className="panel enterprise-teams">
            <div>
              ...
              <button
                aria-label="Add team"
                onClick={() => props.onToast("Team creation opened")}
              >
                +
              </button>
            </div>
            {[
              ...
              return (
                <button
                  key={name as string}
                  onClick={() => props.onToast(`${name} team opened`)}
                >
                  <span>
                    <TeamIcon />
                  </span>
...
function SettingsPage(props: VerusAppProps) {
  return (
    <>
      <PageHeading
        eyebrow="DEMO & ACCESSIBILITY"
        title="Settings"
        copy="Control readability, privacy defaults, and complete product scenarios."
      />
      <div className="settings-layout">
        <section className="panel settings-panel">
          ...
          <SettingToggle
            title="Larger text"
            copy="Increase essential interface text."
            enabled={props.largeText}
            onChange={props.onLargeText}
          />
          <SettingToggle
            title="High contrast"
            copy="Increase panel and text separation."
            enabled={props.highContrast}
            onChange={props.onHighContrast}
          />
          <SettingToggle
            title="Reduce motion"
            copy="Also follows the device setting."
            enabled={false}
            onChange={() => props.onToast("Reduced motion preference saved")}
          />
          <h2>Privacy</h2>
          <SettingToggle
            title="Local behavior view"
            copy="Explicit baseline signals stay on this device."
            enabled
            onChange={() => props.onToast("Local behavior setting updated")}
          />
          <button
            className="danger-text-button"
            onClick={() => props.onToast("Demo data deleted")}
          >
            <Trash2 /> Delete all demo and behavior data
          </button>
        </section>
        <section className="panel scenario-lab">
          ...
          <div className="scenario-list">
            {scenarios.map(([name, detail, outcome]) => (
              <button
                key={name}
                onClick={() => props.onScenario(name, outcome)}
              >
    ...
    <div className="safari-app">
      <div className="browser-toolbar">
        ...
        <div className="address-bar">
          <Lock size={13} /> mail.verus-demo.local
        </div>
        <RefreshCw size={15} />
      </div>
      <div className="mail-app">
        ...
        <div className="mail-list">
          ...
          <button
            className={emailOpened ? "selected unread" : "unread"}
            onClick={onOpenEmail}
          >
          ...
          <button>
            <span className="sender">Maya Roy</span>
            <strong>Dinner on Sunday</strong>
            <p>I made a reservation for 6:30. Does that work?</p>
            <time>9:14 AM</time>
          </button>
          <button>
            <span className="sender">Verus</span>
            <strong>Your weekly privacy summary</strong>
            <p>89 trackers blocked and one old account found.</p>
            <time>Yesterday</time>
          </button>
        </div>
            ...
            <div className="phishing-email">
              <div className="email-head">
                ...
                <div>
                  ...
                  <p>
                    Account Security &lt;security@appleid-review.example&gt;
                  </p>
                </div>
              </div>
              <p>Hello Sparsh,</p>
              <p>
                Your account will be permanently locked today because of an
                unusual sign-in. Confirm your password immediately to prevent
                loss of access.
              </p>
  ...
  return (
    <div className="messages-app">
      <aside>
        ...
        <button className="selected">
          ...
          <div>
            ...
            <p>This is Daniel. New phone...</p>
          </div>
        </button>
        <button>
          <div className="contact-avatar">MR</div>
          <div>
            <strong>Maya Roy</strong>
      ...
      <section>
        <header>
          <strong>Unknown Number</strong>
          <small>Claims to be Daniel Roy</small>
        </header>
        <div className="conversation">
          <span className="message incoming">
            Hi Grandpa, this is Daniel. I lost my phone and I am in trouble.
          </span>
          <span className="message incoming">
            Please buy $600 in gift cards right now. Do not call Mom.
          </span>
          <div className="message-warning">
            <img src="/verus-logo.png" alt="" />
            <div>
              <strong>Contact method does not match</strong>
              <p>This number is not saved for Daniel.</p>
            </div>
...
function InterruptionOverlay({
  ...
  const claimedInitials =
    claimedFirstName === "the caller"
      ? "?"
      : claimedSender
          .split(" ")
          .map((n) => n[0])
    ...
    >
      ...
      >
        <div className="interruption-brand">
          <img src="/verus-logo.png" alt="Verus" />
          <span>
            <strong>VERUS INTERRUPTION</strong>
            <small>Independent verification required</small>
          </span>
        ...
        {state === "PAUSED" && (
          <>
            ...
            <p className="interruption-lead">
              This request will remain paused until it is independently
              confirmed. There is no rush.
            </p>
            <div className="interruption-details">
              <div>
                <small>CLAIMS TO BE</small>
                <strong>{claimedSender}</strong>
                <p>Incoming contact: (917) 555-0382</p>
              </div>
              <div>
                <small>DETERMINISTIC FACT</small>
                <strong>
                  This number is not saved for {claimedFirstName}.
                </strong>
                <p>Saved contact: (609) 555-0162</p>
              </div>
            </div>
            <div className="interruption-signals">
              <SignalTag text="Urgency" />
              <SignalTag text="Secrecy" />
              <SignalTag text="Financial demand" />
              <SignalTag text="Changed contact" strong />
            </div>
            <div className="safe-phrase-guidance">
              <KeyRound />
              <div>
                <strong>Use your family safe-phrase question</strong>
                <p>
                  Ask them for the phrase. Verus will never display it on
                  screen.
                </p>
              </div>
            </div>
            <div className="interruption-actions">
              <button className="primary-button" onClick={onStart}>
                <UserRoundCheck /> Ask {claimedFirstName} to confirm
              </button>
              <button className="secondary-button" onClick={onEnd}>
                <X /> End this interaction
              </button>
            </div>
          </>
        )}
        {state === "PENDING" && (
          <div className="pending-view">
            <span className="pending-visual">
              <UserRoundCheck />
            </span>
            <small className="eyebrow">INDEPENDENT CHECK IN PROGRESS</small>
            <h1>
              {claimedFirstName === "the caller"
                ? "The saved contact is being contacted using the saved number."
                : `${claimedFirstName} is being contacted using the saved number.`}
            </h1>
            <p>
              The original request remains paused. You do not need to stay on
              this screen or respond to the sender.
            </p>
            ...
            {trustedReply && (
              ...
              >
                <div className="trusted-response-head">
                  ...
                  <div>
                    <small>TRUSTED CONTACT VIEW</small>
                    <strong>Is this request really from you?</strong>
                  </div>
                </div>
                <p>
                  A request claiming to be from {claimedFirstName} asks Sparsh
                  for money from a new phone number.
                </p>
                <div>
        ...
        {["DENIED", "VERIFIED", "EXPIRED"].includes(state) && (
          <div className={`resolved-view ${state.toLowerCase()}`}>
            <span className="resolved-icon">
              {state === "VERIFIED" ? <BadgeCheck /> : <ShieldCheck />}
            </span>
            <small className="eyebrow">VERIFICATION COMPLETE</small>
            <h1>
              {state === "VERIFIED"
                ? `${claimedFirstName} confirmed the request.`
                : state === "EXPIRED"
                  ? "No response received. The action remains blocked."
                  : `${claimedFirstName} did not send this request.`}
            </h1>
            <p>
              {state === "VERIFIED"
                ? "The request may continue using the independently confirmed details."
                : "No money, password, or account access was shared."}
            </p>
            <div className="resolution-summary">
              <span>
                <Check /> Risky action{" "}
                {state === "VERIFIED" ? "cleared" : "blocked"}
              </span>
              <span>
                <Check /> Original message not stored
              </span>
              <span>
                <Check /> Safety Receipt{" "}
                {receiptReady ? "created" : "available"}
              </span>
            </div>
            <button className="primary-button" onClick={onClose}>
              View Safety Receipt <ArrowRight />
            </button>
          </div>
        )}
      </motion.section>
