...
test.describe("analyze flow", () => {
  test("home page loads with message input", async ({ page }) => {
    ...
    await expect(page.locator("textarea, [role='textbox'], input[type='text']").first()).toBeVisible();
  });
  ...
  test("submitting a low-risk message shows a result", async ({ page }) => {
    ...
    const button = page.getByRole("button", { name: /check|analyze|submit/i }).first();
    await button.click();

    // Should show a result — either a status level or the result card
    // L0 messages should not show a hold/warn banner
    await expect(page.locator("[data-testid='result'], .result-card, .check-result").first()).toBeVisible({
      timeout: 10_000,
    }).catch(async () => {
  ...
  test("submitting a high-risk message shows a hold instruction", async ({
    ...
    await expect(page.locator("body")).toContainText(/hold|stop|verify|L[23]/i, {
      timeout: 15_000,
    });
  });
  ...
  test("setup page renders enrollment form", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /save|verify|enroll/i }).first()).toBeVisible();
  });
});
